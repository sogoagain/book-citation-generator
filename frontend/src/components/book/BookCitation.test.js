...
describe('BookCitation', () => {
  ...
  function renderBookCitation({ label, citation, copyable } = { label: '', citation: '', copyable: true }) {
    render(<BookCitation
      label={label}
      citation={citation}
      onCopy={handleCopy}
  ...
  it('renders citation', () => {
    const label = 'MLA';
    const citation = '앤드류 헌트, 데이비드 토머스. 실용주의 프로그래머. 인사이트, 2014.';
  ...
  context('when copying is possible', () => {
  ...
  context('when copying is impossible', () => {
